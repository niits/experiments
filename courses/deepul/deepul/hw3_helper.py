...
def q1_save_results(part, fn):
    ...
    # loss plot
    plot_gan_training(
        losses, "Q1{} Losses".format(part), "results/q1{}_losses.png".format(part)
    )
    ...
    q1_gan_plot(
        ...
        xs1,
        ys1,
        "Q1{} Epoch 1".format(part),
        "results/q1{}_epoch1.png".format(part),
    )
    q1_gan_plot(
        data,
        samples_end,
        xs_end,
        ys_end,
        "Q1{} Final".format(part),
        "results/q1{}_final.png".format(part),
    )
...
def q3_save_results(fn, part):
    ...
    gan_losses, lpips_losses, l2_train_losses, l2_val_losses, recon_show = fn(
        train_data, test_data, test_data[:100]
    )

    plot_gan_training(
        gan_losses, f"Q3{part} Discriminator Losses", f"results/q3{part}_gan_losses.png"
    )
    save_plot(
        l2_train_losses,
        l2_val_losses,
        f"Q3{part} L2 Losses",
        f"results/q3{part}_l2_losses.png",
    )
    save_plot(
        lpips_losses,
        None,
        f"Q3{part} LPIPS Losses",
        f"results/q3{part}_lpips_losses.png",
    )
    show_samples(
        test_data[:100].transpose(0, 2, 3, 1) * 255.0,
        nrow=20,
        fname=f"results/q3{part}_data_samples.png",
        title=f"Q3{part} CIFAR10 val samples",
    )
    show_samples(
        recon_show * 255.0,
        nrow=20,
        fname=f"results/q3{part}_reconstructions.png",
        title=f"Q3{part} VQGAN reconstructions",
    )
...
def get_colored_mnist(data):
    ...
    # Resize
    batch_resized = np.asarray(
        [scipy.ndimage.zoom(image, (2.3, 2.3, 1), order=1) for image in data]
    )
    ...
    for i in range(data.shape[0]):
        ...
        batch[i] = cv2.resize(
            image, (0, 0), fx=28 / 64, fy=28 / 64, interpolation=cv2.INTER_AREA
        )
...
def visualize_cyclegan_datasets():
    ...
    show_samples(
        colored_mnist.transpose([0, 2, 3, 1]) * 255.0, title=f"Colored MNIST samples"
    )
