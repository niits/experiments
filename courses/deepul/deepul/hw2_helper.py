...
def q1_save_results(part, dset_id, fn):
    train_data, test_data = q1_sample_data(part, dset_id)
    train_losses, test_losses, samples_noise, samples_nonoise = fn(
        train_data, test_data, part, dset_id
    )
    print(
...
def q3_save_results(dset_id, fn):
    ...
    (
        ...
        samples,
        reconstructions,
    ) = fn(train_data, test_data, dset_id)
    samples, reconstructions = samples.astype("float32"), reconstructions.astype(
        "float32"
    )
    ...
    save_training_plot(
        vqvae_train_losses,
        vqvae_test_losses,
        f"Q3 Dataset {dset_id} VQ-VAE Train Plot",
        f"results/q3_dset{dset_id}_vqvae_train_plot.png",
    )
    save_training_plot(
        prior_train_losses,
        prior_test_losses,
        f"Q3 Dataset {dset_id} Transformer Prior Train Plot",
        f"results/q3_dset{dset_id}_prior_train_plot.png",
    )
    show_samples(
        samples,
        title=f"Q3 Dataset {dset_id} Samples",
        fname=f"results/q3_dset{dset_id}_samples.png",
    )
    show_samples(
        reconstructions,
        title=f"Q3 Dataset {dset_id} Reconstructions",
